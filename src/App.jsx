...
function isValidEmail(value) {
  ...
  return atIndex > 0 && dotIndex > atIndex + 1 && dotIndex < text.length - 1;
}
...
function saveLocalLead(payload) {
  try {
    const raw = window.localStorage.getItem("airdesign_quote_leads");
    const current = raw ? JSON.parse(raw) : [];
    window.localStorage.setItem("airdesign_quote_leads", JSON.stringify([payload, ...current].slice(0, 100)));
    window.localStorage.setItem("airdesign_last_quote_payload", JSON.stringify(payload));
  } catch {
    // Không chặn quy trình nếu trình duyệt không cho lưu localStorage.
  }
}
...
export default function App() {
  ...
  const buildLeadPayload = () => ({
    ...
    project: {
      ...
      constructionArea: result.constructionArea,
      floors: result.floors,
      layoutLabel: result.layoutLabel,
      balconyDepth: result.balconyDepth,
      hasRooftop: result.hasRooftop,
    },
    pricing: {
      foundationUnitPrice: toNumber(input.foundationUnitPrice),
      constructionUnitPrice: toNumber(input.constructionUnitPrice),
      totalBillableArea: result.totalBillableArea,
      totalAmount: result.totalAmount,
      rows: result.rows,
    },
    source: "AirDesign Landing Page",
    pageUrl: typeof window !== "undefined" ? window.location.href : "",
    createdAt: new Date().toISOString(),
  });

  const saveQuoteAndPrint = async () => {
    if (!isValidEmail(input.customerEmail)) {
      setMessage("Vui lòng nhập Gmail/email hợp lệ trước khi lưu bảng báo giá.");
      return;
    }

    if (!isValidPhone(input.customerPhone)) {
      setMessage("Vui lòng nhập số điện thoại hợp lệ trước khi lưu bảng báo giá.");
      return;
    }

    if (result.errors.length > 0) {
      setMessage("Vui lòng kiểm tra lại thông tin công trình trước khi lưu bảng báo giá.");
      return;
    }
    ...
    await Promise.allSettled([
      postLead(GOOGLE_SHEET_WEBAPP_URL, payload),
      postLead(CRM_WEBHOOK_URL, payload),
    ]);

    const hasEndpoint = Boolean(GOOGLE_SHEET_WEBAPP_URL || CRM_WEBHOOK_URL);
    setMessage(
      hasEndpoint
        ? "Đã ghi nhận thông tin khách hàng. Trong hộp thoại in, chọn Save as PDF / Lưu thành PDF."
        : "Đã lưu thông tin trên trình duyệt. Chưa cấu hình Google Sheet/CRM nên chưa gửi lead về hệ thống. Trong hộp thoại in, chọn Save as PDF / Lưu thành PDF."
    );
